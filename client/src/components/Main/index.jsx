...
const Main = () => {
	...
	useEffect(() => {
		const fetchImages = async () => {
			try {
				const response = await axios.get(`https://react-img-api.vercel.app/api/img/images/${userId}`);
	...
	const openLightbox = async (image) => {
		...
		try {
			await axios.post(`https://react-img-api.vercel.app/api/img/${image._id}/view`);
		} catch (error) {
			console.error(error);
		}
	};
